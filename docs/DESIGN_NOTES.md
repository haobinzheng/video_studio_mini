...
### Edit Story: toggle and fresh state
...
- **Export** when **Edit Story** is on, Story mode is selected, and validation passes:
  ...
  - If **Edit Story** is on and validation passes but `makeStoryBlockExportDescriptor()` fails, **AppViewModel** aborts before export with a clear message (avoids legacy pool-wide story pacing). **Preview** with Edit Story blocks uses up to **180s** of measured narration (not the default **20s** preview cap) so multi-block photo timing can be checked.
  - **Preview TTS sample (Edit Story):** `VideoExporter.applyPreviewNarrationSynthesisBudget` must keep **one utterance-index range per story block** (clip each range to the synthesized prefix). Dropping blocks broke `storyBlockUtteranceRanges` alignment and surfaced **invalidStoryBlockPlan** in the UI even when blocks were fully assigned.
  - **Full export after full-length narration prep (Edit Story):** `prepareNarrationPreview` writes `Documents/NarrationPreview/utterance-preview-*.caf`. **`exportVideo`** used to re-synthesize every utterance in the export workspace because the merged M4A path cannot drive per-block timing. When **`narrationPreviewIsFullLength`** and the Edit Story descriptor are valid, **`prebuiltUtteranceSourceDirectory`** points at **`NarrationPreview`** and **`VideoExporter`** **copies** those CAFs into `RenderedVideos/<exportArtifactID>/` as `utterance-*.caf` instead of running TTS again—so **Stop** and **Create Video** again does not redo hundreds of segments if the cache is still valid.
  - **Export workspace:** each run writes under **`Documents/RenderedVideos/<UUID>/`** (final `.mov` plus intermediates such as slideshow and per-utterance audio) so concurrent or overlapping runs do not overwrite the same filenames.
...
- **Stop** while **Preview Video** or **Create Video** runs: **`stopActiveVideoRender()`** cancels the active render task (export runs in-process **`await exportVideo`**, not a detached task), clears progress flags, and sets status to **Render stopped…** In-flight **`AVAssetExportSession`** work is cancelled via **`cancelExport()`** when the task is cancelled. **`activeVideoRenderSessionID`** is cleared **before** cancel so in-flight **`VideoExporter`** / narration **progressHandler** updates (which may still fire briefly) do not overwrite the status line with **Rendering…** again. Stopping **Preview Video** also clears narration preview state so the next final export does not reuse a stale short preview.
